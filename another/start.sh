...
./wait-deps.sh &&\
    ...
    /src/matchengine/restart.sh &&\
    /src/wait-for-it.sh 127.0.0.1:7316 --timeout=60 --quiet --strict -- \
    echo ">>> matchengine started on 7316"

echo ">>> readhistory starting" &&\
    /src/readhistory/restart.sh &&\
    /btc/wait-for-it.sh 127.0.0.1:7424 --timeout=60 --quiet --strict -- \
    echo ">>> readhistory started on 7424"

echo ">>> alertcenter starting..." &&\
    /src/alertcenter/restart.sh &&\
    /src/wait-for-it.sh 127.0.0.1:4444 --timeout=60 --quiet --strict -- \
    echo ">>> alertcenter started on 4444"

echo ">>> accesshttp starting..." &&\
    /src/accesshttp/restart.sh &&\
    /src/wait-for-it.sh 127.0.0.1:8080 --timeout=60 --quiet --strict -- \
    echo ">>> accesshttp started on 8080"


echo ">>> accessws starting..." &&\
    /src/accessws/restart.sh &&\
    /src/wait-for-it.sh 127.0.0.1:8091 --timeout=60 --quiet --strict -- \
    echo ">>> accessws started on 8091"

echo ">>> marketprice starting..." &&\
    /src/marketprice/restart.sh &&\
    /src/wait-for-it.sh 127.0.0.1:7416 --timeout=60 --quiet --strict -- \
