# author: seref@ercelik.net
# python3 ile yazilmistir!
...
class ViaBTCAPI(object):
    ...
    def order_deals(self, order_id=2, offset=0, limit=10):
...
#         return self._execute("order.book", [market, side, 0, limit])
    ...
    def order_pending(self, user_id=1, market='BTCBCH', offset=0, limit=10):
    ...
    def market_status(self, market="BTCBCH", period=86400):
    ...
    def order_put_stop_limit(
        ...
        return self._execute(
            ...
            [user_id, market, _side, str(trigger), str(amount), str(price),
            str(taker_fee_rate), str(maker_fee_rate), source]
        )


    # Added new Features
    def order_put_stop_market(
            self,
            user_id=2,
            market="BTCETH",
            side="SELL",
        ...
        _side = self._get_side_code(side)
        return self._execute(
            "order.put_stop_market",
            [user_id, market, _side, str(trigger), str(amount),
            str(taker_fee_rate), str(maker_fee_rate)]
        )
